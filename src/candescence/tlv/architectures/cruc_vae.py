"""
Purpose: Conditional Routed U-Net VAE (CRUC VAE)
Author: Hallett Lab
Date: 2026-01-27
Input: Images and conditioning vectors
Output: Reconstructed images, latent representations

Uses FiLM conditioning with 1x1 conv skip reduction and latent-augmented
skip connections routed through the decoder.
"""
...
logger = get_logger("candescence.architectures.cruc_vae")


class Encoder(nn.Module):
    """
    Conditional encoder with 1x1 conv skip reduction and latent augmentation.

    Skip connections are reduced to latent_dim channels via 1x1 convolutions,
    then augmented with the latent vector z expanded spatially.
    """
    ...
    def __init__(self, arguments: dict) -> None:
        ...
        # 1x1 convolutions to reduce feature map channels
        self.reduce_64 = nn.Conv2d(64, self.latent_dim, kernel_size=1)
        self.reduce_128 = nn.Conv2d(128, self.latent_dim, kernel_size=1)
        self.reduce_256 = nn.Conv2d(256, self.latent_dim, kernel_size=1)
        self.reduce_512 = nn.Conv2d(512, self.latent_dim, kernel_size=1)

        # FC layers initialized dynamically in cruc_VAE
    ...
    def forward(
        ...
        # Reduce feature maps using 1x1 convolutions
...
class Decoder(nn.Module):
    """
    Conditional decoder with FiLM modulation on projected conditioning.

    Receives latent-augmented skip connections from the encoder.
    """
    ...
    def __init__(self, arguments: dict) -> None:
        ...
        # Decoder layers (input channels = latent_dim * 2 for augmented skips)
        self.convtrans3 = nn.ConvTranspose2d(self.latent_dim * 2, 256, 3, stride=2, padding=1, output_padding=1)
        self.bn3 = nn.BatchNorm2d(256)

        self.convtrans2 = nn.ConvTranspose2d(256 + 2 * self.latent_dim, 128, 3, stride=2, padding=1, output_padding=1)
        self.bn2 = nn.BatchNorm2d(128)

        self.convtrans1 = nn.ConvTranspose2d(128 + 2 * self.latent_dim, 64, 3, stride=2, padding=1, output_padding=1)
    ...
    def forward(
        ...
        *args: torch.Tensor
        ...
        skip_connections : list
            Latent-augmented skip connections [skip1, skip2, skip3, skip4]
        *args : torch.Tensor
...
class cruc_VAE(nn.Module):
    """
    Conditional Routed U-Net VAE (CRUC VAE).

    Uses 1x1 conv skip reduction, latent-augmented skip connections,
    and FiLM conditioning on projected latent vectors.
