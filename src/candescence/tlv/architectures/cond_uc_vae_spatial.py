"""
Purpose: Conditional U-Net VAE with spatial filtering on skip connections
Author: Hallett Lab
Date: 2026-01-27
Input: Images and conditioning vectors
Output: Reconstructed images, latent representations

Adds learned spatial filter blocks to skip connections in the encoder,
applying a downsample-process-upsample pipeline that filters spatial
information before passing to the decoder.
"""
...
logger = get_logger("candescence.architectures.cond_uc_vae_spatial")


class SpatialFilterBlock(nn.Module):
    """
    Learned spatial filter that downsamples, processes, and upsamples feature maps.
    ...
    reduction_ratio : int
        Channel reduction ratio (default: 16)
    spatial_reduction : int
        Spatial downsampling stride (default: 2)
    """
    ...
    def __init__(
        ...
        spatial_reduction: int = 2
    ) -> None:
        super().__init__()

        hidden = max(1, in_channels // reduction_ratio)

        # Downsample channels and spatial dims
        self.conv_down = nn.Conv2d(
            in_channels, hidden,
            kernel_size=3, stride=spatial_reduction, padding=1, bias=False
        )
        self.bn_down = nn.BatchNorm2d(hidden)
        ...
        self.conv_up = nn.ConvTranspose2d(
            hidden, in_channels,
            kernel_size=2 * spatial_reduction,
            stride=spatial_reduction,
            padding=spatial_reduction // 2,
            bias=False
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = self.relu(self.bn_down(self.conv_down(x)))
        out = self.relu(self.bn_mid(self.conv_mid(out)))
        out = self.bn_up(self.conv_up(out))
        return out


class Encoder(nn.Module):
    """
    Conditional encoder with FiLM modulation and spatial filtering on skips.

    Spatial filter blocks are applied to each encoder level's feature maps
    before they are passed as skip connections to the decoder.
    """
    ...
    def __init__(self, arguments: dict) -> None:
        ...
        # Parse per-level reduction ratios and spatial reductions
        rr = self.arguments.get('reduction_ratio', 16)
        sr = self.arguments.get('spatial_reduction', 2)
        if not isinstance(rr, (list, tuple)):
            rr = [rr] * 4
        if not isinstance(sr, (list, tuple)):
            sr = [sr] * 4

        # Spatial filter blocks per encoder stage
        self.spatial1 = SpatialFilterBlock(64, rr[0], sr[0])
        self.spatial2 = SpatialFilterBlock(128, rr[1], sr[1])
        self.spatial3 = SpatialFilterBlock(256, rr[2], sr[2])
        self.spatial4 = SpatialFilterBlock(512, rr[3], sr[3])
    ...
    def forward(
        ...
        Returns z, mu, logvar, skip_connections (spatially filtered), feature_map_shape
        """
...
class Decoder(nn.Module):
    """Conditional decoder with FiLM modulation (same as cond_uc_vae decoder)."""
    ...
    def forward(
        ...
        """Forward pass with spatially-filtered skip connections."""
...
class cond_uc_VAE_spatial(nn.Module):
    """
    Conditional U-Net VAE with spatial filtering on skip connections.

    Adds learned spatial filter blocks (downsample-process-upsample) to
    each encoder level's skip connections before passing to the decoder.

    Parameters
    ----------
    arguments : dict
        Configuration dictionary. Supports:
        - 'reduction_ratio': int or list of 4 ints (channel reduction)
        - 'spatial_reduction': int or list of 4 ints (spatial downsampling stride)
    """

    def __init__(self, arguments: dict) -> None:
        super(cond_uc_VAE_spatial, self).__init__()

        logger.info(f"reduction_ratio   = {arguments.get('reduction_ratio', 16)}")
        logger.info(f"spatial_reduction = {arguments.get('spatial_reduction', 2)}")
